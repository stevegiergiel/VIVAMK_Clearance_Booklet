#!/usr/bin/env python3
# VIVAMK CLEARANCE CHRISTMAS VERSION v1.11
# Updated: 2026-08-09 03:28 +0100
# Changes: Moves the overlaid EzeGet QR slightly higher so the original blue bottom border remains visible.
# Changes: Retains the improved clearer QR overlay, v1.07 order wording, and the larger/bold SKU plus sale/product QR logic.
"""
...
def apply_config(cfg: dict):
    ...
    global PRODUCT_CODE_LABEL, PRODUCT_CODE_FONT_SIZE, PRODUCT_CODE_BOLD
    ...
    for i, row in enumerate(rows, 1):
        ...
        if row.sku in cache and not refresh:
            ...
            if row.product_url:
                ...
                row.qr_file = make_qr(row.product_url, image_dir / f"{row.sku}_qr.png")
        ...
        if row.product_url:
            row.product_url = personal_vivamk_url(row.product_url)
            if not row.qr_file or not Path(row.qr_file).exists():
                row.qr_file = make_qr(row.product_url, image_dir / f"{row.sku}_qr.png")
                cache[row.sku] = asdict(row)
                data_file.write_text(json.dumps(cache, indent=2, ensure_ascii=False), encoding="utf-8")
...
def draw_product_card(c: canvas.Canvas, row: SaleRow, x: float, y: float, w: float, h: float):
    ...
    qr_size = PRODUCT_QR_SIZE_MM * mm
    qr_left = right - qr_size
    price_right = qr_left - 1.2 * mm if row.qr_file and Path(row.qr_file).exists() else right
    ...
    c.drawString(left, panel_y + 2.1 * mm, f"{PRODUCT_CODE_LABEL} {row.sku}")

    if row.qr_file and Path(row.qr_file).exists():
        c.drawImage(
            row.qr_file,
            qr_left,
            panel_y + 1.1 * mm,
